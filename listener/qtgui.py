...
class ListenerMain(QtGui.QMainWindow):
    ...
    def __init__(self, *args, **named):
        ...
        self.context = context.Context(
            getattr(command_line_arguments, 'context', 'default')
        )
    ...
    def create_gui(self):
        ...
        self.view_frame.javaScriptWindowObjectCleared.connect(self.add_gui_bridge)
    ...
    def main_view_html(self):
        return MAIN_PAGE_TEMPLATE.render(view=self, HERE=os.path.abspath(HERE),)
    ...
    def on_level(self, record):
        ...
        *seems* like the pocketsphinx stuff works fine with full-volume
        ...
        intensity = (intensity - self.ZERO_LEVEL_AUDIO) / (
            self.FULL_LEVEL_AUDIO - self.ZERO_LEVEL_AUDIO
        )
        translated = min((1.0, max((0, intensity))))
        js = 'recording_level( %f )' % (translated,)
        self.view_frame.evaluateJavaScript(js)

    def on_partial(self, record):
        for record in self.interpreter(record):
            self.statusBar().showMessage(record['interpreted'])
            self.proxy.send_partial(
                record['interpreted'], record['text'], record['uttid']
            )
    ...
    def on_final(self, record):
        for record in self.interpreter(record):
            ...
            self.systray.showMessage(
                'Recognized', unicode(record['interpreted']), msecs=500
            )
            if isinstance(record['interpreted'], (bytes, unicode)):
                self.proxy.send_final(
                    record['interpreted'], record['text'], record['uttid']
                )
            else:
    ...
    def on_systray(self, reason):
        if self.pipeline.running:
            ...
            self.systray.showMessage(
                "Listener", "Shut down Listener Pipeline, click to re-start"
            )
        else:
            self.pipeline.start_listening()
            self.systray.setIcon(QtGui.QIcon.fromTheme('media-playback-stop'))
            self.systray.showMessage(
                "Listener",
                "Restarted Listener Pipeline for %s, click to stop"
                % (self.context.key,),
            )
        return False
