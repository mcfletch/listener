...
def get_espeak(word, voice='en-us'):
    ...
    return kill_speaking_cues(
        subprocess.check_output(['espeak', '-q', '--ipa=3', '-v', voice, word,]).decode(
            'utf-8'
        )
    )
...
def _stat_translate(ipa):
    ...
    ipa = [c.strip() for c in ipa.split(u'_')]
    results = []
    for sound in ipa:
        if not sound:
            continue
...
def frequency_table(count_table, threshold=0.05):
    ...
    for (ipa, counts) in count_table.items():
        ...
        counts = [
            x
            for x in [(p, count / total) for (p, count) in counts]
            if x[1] >= threshold
        ]
        result[ipa] = counts
    return result
...
def create_stat_mapping():
    ...
    try:
        ...
        table = json.dumps(
            {u'lines': i, u'mapping': frequency_table(mapping),},
            indent=2,
            ensure_ascii=False,
        ).encode('utf-8')
...
def test():
    ...
    try:
        for i, line in enumerate(open(dictionary)):
            ...
            try:
                ...
                print(
                    (
                        ...
                        % (word, err.args[1], description, err.args[0], err.args[0],)
                    )
                )
                raise
            ...
            if not i % 1000:
                print(
                    'good=%s bad=%s close=%s good=%0.3f (good_or_close=%0.3f) avg choices %0.1f'
                    % (
                        good,
                        bad,
                        close_count,
